<?php
// File: /admin/supply-partners-action.php (FINAL & FIXED)
...
if (isset($_POST['activate_supply_partner'])) {
    ...
    // Ambil nama username dari publisher
    ...
    // PERBAIKAN: Tipe data diubah dari "isss" menjadi "iss"
    $stmt->bind_param("iss", $user_id, $partner_name, $supply_key);

    if ($stmt->execute()) {
        redirect_with_message('success', 'Publisher has been activated as an RTB Supply Partner. Please set their Default Zone.');
    } else {
...
// AKSI BARU: Mengatur Zona Default
