<?php
// File: /admin/supply-partners.php (FINAL - With Correct Domain Setting)
...
// PERBAIKAN: Menggunakan domain dari settings untuk membangun base URL
...
<div class="modal fade" id="setDefaultZoneModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Set Default Zone for Stats</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><form action="supply-partners-action.php" method="POST"><div class="modal-body"><input type="hidden" name="source_id" id="modal_source_id"><p>Select a zone where all statistics from this supply source will be recorded.</p><div class="mb-3"><label class="form-label">Default Zone</label><select class="form-select" name="zone_id" required><option value="">Choose a zone...</option><?php foreach ($all_zones as $zone): ?><option value="<?php echo $zone['id']; ?>"><?php echo htmlspecialchars($zone['name']); ?></option><?php endforeach; ?></select></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" name="set_default_zone" class="btn btn-primary">Set Zone</button></div></form></div></div></div>
