<?php
// File: /publisher/sites.php (FINAL - Fixed JS errors and conditional buttons)
...
$sites_result = $conn->query("SELECT id, url, status FROM sites WHERE user_id = {$publisher_id} ORDER BY url ASC");
...
<div class="alert alert-success alert-dismissible fade show" role="alert"><i class="bi bi-check-circle-fill me-2"></i><?php echo $_SESSION['success_message']; ?><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
<?php unset($_SESSION['success_message']); endif; ?>

<?php if (isset($_SESSION['error_message'])): ?>
<div class="alert alert-danger alert-dismissible fade show" role="alert"><i class="bi bi-exclamation-triangle-fill me-2"></i><?php echo $_SESSION['error_message']; ?><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
<?php unset($_SESSION['error_message']); endif; ?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="mt-4 mb-0">My Sites & Zones</h1>
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addSiteModal"><i class="bi bi-plus-circle-fill"></i> Add New Site</button>
</div>
                ...
                <div class="accordion-body">
                    <?php if ($site['status'] == 'approved'): ?>
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="mb-0">Ad Zones</h6>
                            <button class="btn btn-sm btn-outline-primary add-zone-btn" data-bs-toggle="modal" data-bs-target="#addZoneModal" data-site-id="<?php echo $site['id']; ?>"><i class="bi bi-plus"></i> Add Zone</button>
                        </div>
                        <?php
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered">
                                <thead><tr><th>Zone Name</th><th>Ad Format</th><th>Size</th><th>Actions</th></tr></thead>
                                <tbody>
                                    ...
                                    <?php endwhile; else: ?>
                                    <tr><td colspan="4" class="text-center">No zones configured for this site. Click 'Add Zone' to create one.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <?php elseif ($site['status'] == 'pending'): ?>
                        <p class="text-muted">This site is awaiting approval. You can add zones after it has been approved by an administrator.</p>
                    <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inisialisasi event listener untuk semua tombol yang ada
    document.body.addEventListener('click', function(event) {
        const baseAdServerUrl = '<?php echo rtrim($base_ad_server_url, '/'); ?>';
        
        // Handle "Get Display Tag"
        if (event.target && event.target.classList.contains('get-tag-btn')) {
        ...
        // Handle "Get VAST Tag"
        if (event.target && event.target.classList.contains('get-vast-tag-btn')) {
    ...
    // Handle conditional display of Ad Size dropdown
    ...
    if(formatSelectorPub && sizeContainerPub && sizeSelectorPub) {
        formatSelectorPub.addEventListener('change', function() {
        ...
        // Trigger change on load in case a value is pre-selected
        formatSelectorPub.dispatchEvent(new Event('change'));
    }
});
</script>
